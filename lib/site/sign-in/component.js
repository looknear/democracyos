...
class SignIn extends Component {
  ...
  handleFail = (err, code) => {
    ...
    } else if (err && err.toString() === 'Incorrect username') {
      this.setState({loading: false, errors: [t('common.no-user-for-email')]})
    } else if (err && err.toString() === 'Incorrect password') {
      this.setState({loading: false, errors: [t('invalid.password')]})
    } else {
      this.setState({loading: false, errors: err})
    }
  }

  updateValidationMsg (el) {
    console.log(el)
    el.setCustomValidity(t('validators.invalid.email'))
  }
